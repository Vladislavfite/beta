...
function spawnEnemy(scene) {
    ...
    e.maxHp = 100; e.hp = e.maxHp; e.speed = 0.2; e.path = path; e.pathIndex = 1;
...
function updateEnemy(e) {
    ...
    if(e.targetTower && e.targetTower.active){
        e.state='attack'; moveTowards(e,e.targetTower.x,e.targetTower.y,e.speed);
        e.setFlipX(e.targetTower.x<e.x); e.setFlipY(false);
        ...
        if(d<26 && (!e._lastAttack || Date.now()-e._lastAttack>800)){
            ...
            if(e.targetTower.hp!=null){ e.targetTower.hp-=10; try{e.targetTower.setTint(0xff9999); setTimeout(()=>e.targetTower.clearTint(),80);}catch(e){} 
                if(e.targetTower.hp<=0){ e.targetTower.destroy(); towers=towers.filter(t=>t.sprite&&t.sprite.active); e.targetTower=null; e.state='returning'; if(e._savedPathIndex!=null)e.pathIndex=e._savedPathIndex;} 
    ...
    if(e.state==='returning'){ const target=e.path[e.pathIndex]||e.path[e.path.length-1]; moveTowards(e,target[0],target[1],e.speed); e.rotation=Phaser.Math.Angle.Between(e.x,e.y,target[0],target[1]); e.setFlipY(false); if(Phaser.Math.Distance.Between(e.x,e.y,target[0],target[1])<8){ e.state='walk'; if(e.anims && (!e.anims.currentAnim || e.anims.currentAnim.key!=='e_walk_anim')) e.play('e_walk_anim'); } return; }
    ...
    e.state='walk'; const waypoint=e.path[e.pathIndex]; if(waypoint){ moveTowards(e,waypoint[0],waypoint[1],e.speed); e.rotation=Phaser.Math.Angle.Between(e.x,e.y,waypoint[0],waypoint[1]); e.setFlipY(false); if(Phaser.Math.Distance.Between(e.x,e.y,waypoint[0],waypoint[1])<6) e.pathIndex++; } if(e.anims && (!e.anims.currentAnim || e.anims.currentAnim.key!=='e_walk_anim')) e.play('e_walk_anim');
}
