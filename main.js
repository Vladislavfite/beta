...
const BASE_POS = { x:160, y:1005 };       // позиция базы
const BASE_RECT = { w:153, h:93 };        // размеры базы
...
function create_preload() {
  this.load.image('map', 'assets/map.png');                  // фон карты
  this.load.image('molot', 'assets/elements/moloticon.png'); // иконка молота для построек
  this.load.image('up_icon', 'assets/elements/up.png');      // иконка улучшения
  this.load.image('noup_icon', 'assets/elements/noup.png');  // иконка недоступного улучшения
...
function create() {
  ...
  // создание мест для строительства башен
...
function update() {
  ...
  // обновление здоровья базы
  ui.baseBar.width = Math.max(0, 320 * (baseHp / 1000));
  ui.baseBar.fillColor = baseHp > 600 ? 0x00cc00 : (baseHp > 300 ? 0xcccc00 : 0xcc0000);

  // обновление иконок улучшения башен
  for (let t of towers) {
    const ts = t.sprite;
    if (ts && ts.upIcon) {
      const key = (ts.level >= 12) ? null : (gold >= UPGRADE_COST_BASE * ts.level ? 'up_icon' : 'noup_icon');
