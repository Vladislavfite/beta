...
let gold = START_GOLD, wave = 0, canWatchAd = true;
...
function upgradeTower(scene, ts) {
  let num = parseInt(ts._typeKey.replace(/[^0-9]/g, '')) || 1, next = 'tower' + (num + 1);
  if (!scene.textures.exists(next)) { alert('Max upgrade'); return; }
  if (gold < 150) { alert('Need 150 gold'); return; }
  gold -= 150; ui.goldText.setText('Gold:' + gold);
  ts.setTexture(next); ts._typeKey = next; ts.level++; ts.hp += 100; ts._shootRate = Math.max(400, ts._shootRate - 100);
}

function updateBullet(b) {
  if (!b.active || !b.target || !b.target.active) { if (b.active) b.destroy(); return; }
  let dx = b.target.x - b.x, dy = b.target.y - b.y, dist = Math.sqrt(dx * dx + dy * dy);
  if (dist < 8) {
    b.target.hp -= 20;
    if (b.target.hp <= 0) { b.target.destroy(); gold += KILL_REWARD; ui.goldText.setText('Gold:' + gold); }
    b.destroy(); return;
  }
  b.x += (dx / dist) * 10; b.y += (dy / dist) * 10; b.rotation = Math.atan2(dy, dx);
}
...
setInterval(() => {
  try {
    ...
    for (let t of towers) {
      ...
      ts._lastShot += 200; if (ts._lastShot < ts._shootRate) continue; ts._lastShot = 0;
      ...
      if (target) {
        ...
        sc.physics.add.existing(b); b.target = target; bullets.add(b);
      }
    }
