// Full Beta Tower Defense — corrected and cleaned
...
const SPAWNS = [[377,50],[81,335],[636,490],[52,667],[646,963],[160,1005]];
...
// Scene functions are declared below, config references them by name
function create_preload() {
  ...
  // enemy frames (walk / attack / die) — expects up to 7 each
  for (let i = 0; i < 7; i++) {
  ...
  // debug callbacks to see which files load or fail
...
function create() {
  ...
  // mappath as semi-transparent overlay (if exists)
  if (this.textures.exists('mappath')) {
    this.add.image(360, 640, 'mappath').setDisplaySize(720, 1280).setAlpha(0.25);
  }
...
function spawnEnemy(scene) {
  const spawn = SPAWNS.length ? SPAWNS[Math.floor(Math.random() * SPAWNS.length)] : [100, 50];
  // safety check
  if (!Array.isArray(spawn) || spawn.length < 2 || isNaN(spawn[0]) || isNaN(spawn[1])) {
    console.error('Invalid spawn coords:', spawn);
    return;
  }
  ...
  // walk anim cycle (if frames exist)
  let idx = 0;
...
function upgradeTower(scene, ts) {
  ...
  if (gold < 150) { alert('Need 150 gold to upgrade'); return; }
...
// Phaser config and instantiation at the end (to ensure functions exist)
const config2 = {
  type: Phaser.AUTO,
  parent: 'game',
  width: 720,
